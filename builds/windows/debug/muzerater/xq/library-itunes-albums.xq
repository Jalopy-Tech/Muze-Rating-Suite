...
{
let $ratedTracks := $starttracks/local:iTunes(/)/track[ratingComputed != "true" and album != "" and rating != ""]
return(
<albums> {
for $x in distinct-values($ratedTracks/concat(local:assignedAlbumArtist(albumArtist,artist),"||", album))
	...
	let $totalTracks := ($s1 + $s2 + $s3 + $s4 + $s5)
	let $totalTracks2 := $s1 + $s2 + $s3 + $s4 + $s5
	...
	<album>
		...
		<numberOfStars>{$totalStars}</numberOfStars>
		<numberOfTracks>{$totalTracks}</numberOfTracks>
		<starsPerTrack>{$starsPerTrack}</starsPerTrack>
		<uncalculatedRating s1="{$s1}" s2="{$s2}" s3="{$s3}" s4="{$s4}" s5="{$s5}"></uncalculatedRating>
	</album>
	)}
</albums>
)
